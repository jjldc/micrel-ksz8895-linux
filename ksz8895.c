 ...
 *
 * Driver for the PHY-mode Micrel KSZ8895 5-port ethernet switch using SMI
 *
 * Author: Nathan L. Conrad <downwith@users.noreply.github.com>
 *
 * Copyright (c) 2013 Nathan L. Conrad
 *
...
static int ksz8895_probe(struct platform_device *pdev)
{
	...
	ret = mdiobus_register(data->switch_bus);
	if (ret) {
	...
	if (!data->switch_bus->phy_map[KSZ8895_SWITCH_ADDR]->drv) {
	...
	dev_info(data->dev, "Detected PHY driver '%s'\n",
		 data->switch_bus->phy_map[KSZ8895_SWITCH_ADDR]->drv->name);
	return 0;
}
...
static int ksz8895_switch_config_init(struct phy_device *phydev)
{
	...
	struct ksz8895_data *data = phydev->bus->priv;
...
int ksz8895_switch_match_phy_device(struct phy_device *phydev)
{
	return (phydev->phy_id & KSZ8895_PHY_ID_MASK) ==
	       ...
	       phydev->addr == KSZ8895_SWITCH_ADDR;
}
...
int ksz8895_phy_5_match_phy_device(struct phy_device *phydev)
{
	return (phydev->phy_id & KSZ8895_PHY_ID_MASK) ==
	       (KSZ8895_PHY_ID & KSZ8895_PHY_ID_MASK) &&
	       phydev->addr == KSZ8895_PHY_5_ADDR;
}
...
MODULE_DESCRIPTION("Micrel KSZ8895 driver");
MODULE_AUTHOR("Nathan L. Conrad <downwith@users.noreply.github.com>");
MODULE_LICENSE("GPL");
